...
**Zenodo remains unpublished.** Navigation showed HTTP 504 before the upload form; a later retry timed out again. No record, file upload or DOI was created in this session. Prepared archives for logcumsumexp, expm1 and this report remain available from their GERO articles. Before a later upload, inspect My uploads for existing records/drafts to avoid duplicates. For each absent report: upload its archive, describe it, save and preview the draft, then publish. DOI registration occurs on publication; see the [official deposit guide](https://help.zenodo.org/docs/deposit/create-new-upload/).
