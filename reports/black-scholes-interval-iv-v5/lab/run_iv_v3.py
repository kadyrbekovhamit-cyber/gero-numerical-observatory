...
from lab.iv_reference_v3 import check_witness,witnesses,number,value
from lab.iv_corpus_v3 import verify,ROOT


def run(output,portable=False):
    assert not output.exists(),'Use a fresh output path'
    freeze=verify(portable);ctx.threads=1
    cases=json.loads((ROOT/'iv-evidence/corpus-v3.json').read_text());rows=[];start=time.monotonic()
    for case in cases:
