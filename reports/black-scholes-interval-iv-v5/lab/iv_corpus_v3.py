"""Third pre-output corpus, after disclosed reference fixes; same candidate."""
...
from lab.iv_reference_v3 import value
from lab.iv_corpus import ROOT,sha,dependency_manifest

SOURCES=['lab/iv_interval.py','lab/iv_reference_v3.py','lab/iv_corpus_v3.py','lab/run_iv_v3.py',
         'iv-tests/test_iv_interval.py','iv_range.py','EXPERIMENT_IV_V3.md','requirements-iv.txt',
         'lab/iv_corpus.py','iv-evidence/freeze-v1.json','iv-evidence/corpus-v1.json',
         'iv-evidence/confirmation-v1.json','iv-evidence/freeze-v2.json','iv-evidence/corpus-v2.json',
         'iv-evidence/confirmation-v2.json','iv-tests-v3/test_reference_v3.py']
...
def build():
    ...
    def add(family,c,q,status='interval',resolved=True,seed=None,kwargs=None,state=None):
        r={'id':f'iv3-{len(rows)+1:03d}','family':family,'contract':c.as_dict(),'quote':q.as_dict(),
        ...
        rows.append(r)
    for option,scale,t,sigma in product(('call','put'),(F('1e-309'),F('1e120')),
                                       (F(5,8),F(11,4)),(F('.13'),F('.29'),F('.67'),F('1.6'))):
        c=Contract(139*scale,109*scale,t,F(3,200),F(1,40),option)
        add('rounded_recovery',c,rounded_quote(c,sigma),seed=sigma)
    for option,(s,k),which in product(('call','put'),((181,99),(241,121),(105,229),(89,209)),('intrinsic','ceiling')):
    ...
    add('budget_boundary',Contract(117,117,1,r='1e-30'),QuoteInterval('1e-29',9),
        status='unresolved',resolved=False,kwargs={'precisions':[64]})
    for option,s,inside in product(('call','put'),(91,115),(True,False)):
        c=Contract(s,107,0,option=option);floor=max(s-107 if option=='call' else 107-s,0)
    ...
    old={key(r) for v in (1,2) for r in json.loads((ROOT/f'iv-evidence/corpus-v{v}.json').read_text())}
    assert len({key(r) for r in rows})==len(rows) and not old.intersection(key(r) for r in rows)
    return rows


def freeze():
    target=ROOT/'iv-evidence/corpus-v3.json';receipt=ROOT/'iv-evidence/freeze-v3.json'
    ...
    data={'time_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'purpose':'new inputs frozen after v1/v2 diagnosis, before candidate v3-corpus outputs',
...
def verify(portable=False):
    d=json.loads((ROOT/'iv-evidence/freeze-v3.json').read_text())
    for n,h in d['sources'].items():assert sha(ROOT/n)==h,n
    assert sha(ROOT/'iv-evidence/corpus-v3.json')==d['corpus_sha256']
