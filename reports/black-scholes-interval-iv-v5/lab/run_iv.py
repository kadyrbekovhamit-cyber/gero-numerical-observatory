...
from lab.iv_reference import check_witness,witnesses,number,value
from lab.iv_corpus import verify,ROOT


def run(output,portable=False):
    assert not output.exists(),'Use a fresh output path'
    freeze=verify(portable);ctx.threads=1
    cases=json.loads((ROOT/'iv-evidence/corpus-v1.json').read_text());rows=[];start=time.monotonic()
    for case in cases:
