...
class Result:
    ...
    log_truncation_bound_over_integral: float
    branch: str
    underflow: bool
...
def _integrate(function, cuts):
    initial = [(a,b,_rule(function,a,b,RULE16),_rule(function,a,b,RULE32))
               for a,b in zip(cuts,cuts[1:]) if a < b]
    evaluations = 48*len(initial)
    initial_total = math.fsum(fine for _,_,_,fine in initial)
    absolute_budget = QUAD_TOL*initial_total/len(initial)
    def interval(left, right, depth, budget, coarse=None, fine=None):
        nonlocal evaluations
        if coarse is None:
            coarse = _rule(function,left,right,RULE16)
            fine = _rule(function,left,right,RULE32)
            evaluations += 48
            if evaluations > QUAD_MAX_EVALUATIONS:
                raise ArithmeticError('quadrature evaluation budget exhausted')
        error = abs(fine-coarse)
        # Absolute allocation prevents endless refinement of negligible tails;
        # the relative term accommodates the binary64 evaluation noise floor.
        if error <= budget+QUAD_TOL*abs(fine) or fine == coarse:
            return fine,error
        if depth == 12:
            raise ArithmeticError('binary64 quadrature did not converge')
        middle = (left+right)*0.5
        a,ea = interval(left,middle,depth+1,budget*0.5)
        b,eb = interval(middle,right,depth+1,budget*0.5)
        return math.fsum((a,b)),ea+eb
    parts = [interval(a,b,0,absolute_budget,coarse,fine) for a,b,coarse,fine in initial]
...
def prices(s, k, t, r, q, sigma):
    ...
    if v == 0.0 and sigma > 0.0 and t > 0.0:
        raise ArithmeticError('positive total volatility underflows binary64; log-volatility input path required')
    ...
    if v == 0.0:
        return Result(max(parity,0.),max(-parity,0.),parity,option,0.,-math.inf,m,0,0.,-math.inf,'deterministic',False)
    ...
    if z <= 0.0:
        ...
        # Integrand <= w*exp(-w) for z<=-1, or <= w*exp(-w*w/2)
        # for -1<z<=0. Retain the log bound even if its exponential underflows.
        log_tail_bound = math.log(65.)-64. if z<=-1. else -0.5*64.**2
        ...
        # Since (1-exp(-v*u))/v <= u, bound the omitted Gaussian tails.
        tail_multiplier = 1.0+(2.0 if z>12. else 1.0)*z/12.
        log_tail_bound = math.log(tail_multiplier)-72.
        branch = 'centered_positive_z_integral'
    log_tail_ratio = log_tail_bound-math.log(integral)
    if log_tail_ratio > math.log(QUAD_TOL*0.125):
        raise ArithmeticError('fixed integration window has insufficient tail bound')
    ...
    return Result(call,put,parity,option,value,log_value,m,evaluations,estimate,log_tail_ratio,branch,value==0.0)
