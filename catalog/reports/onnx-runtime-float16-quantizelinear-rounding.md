...
[Original GERO article](https://www.gero.uz/research/articles/onnx-runtime-float16-quantizelinear-rounding.html) · [GitHub report](https://github.com/kadyrbekovhamit-cyber/gero-numerical-observatory/tree/main/reports/onnx-runtime-float16-quantizelinear-rounding) · [Original archived collection](https://doi.org/10.5281/zenodo.22683900) · [Individual Zenodo record](https://doi.org/10.5281/zenodo.22729032)
...
Text SHA-256: `a674fc490c1cbd934b9ba84b1d9b202c473ed8eda0bad9c0f9d1ad776880d733`

## Original archived text

<!-- BEGIN UNCHANGED CORPUS TEXT -->
ONNX Runtime float16 QuantizeLinear truncates instead of rounding
https://www.gero.uz/research/articles/onnx-runtime-float16-quantizelinear-rounding.html
...
Reproduced on ONNX Runtime 1.29.0; correction PR #32452 is approved. Its sole red optional-lint job failed while building an unrelated reviewdog container.
...
specification requires round-to-nearest with ties to even. The ONNX Runtime CPU path for
...
These inputs are not exact ties: their magnitudes are greater than

0.5

, so both nearest-even and ordinary nearest rounding select magnitude
...
; the latter was tested with ONNX
...
eebea690a0b842d6c1aae89e3f1b2164a95f9233
...
still uses the direct integer cast in the

MLFloat16

overload, while other nearby paths use

std::nearbyint

.

The runtime call chain reaches that overload through

quantize_linear.cc:1014

,

quantize_linear.cc:822
...
A 4,096-value float16 differential test produced 2,041 mismatches against the specification oracle, always toward zero and by at most one quantization unit. The float32 control returned the expected result.

Control audit:

18/18

float32 cases passed, including ties, saturation, infinities, per-axis broadcasting, dequantization, random differential cases, and scale equivariance.

Existing issue

microsoft/onnxruntime#18576

reports the same float16-to-int8 rounding defect. It was opened on 2023-11-24, independently reconfirmed on version

1.20.1

, and closed by a stale bot on 2025-08-07 without a correction.

The correction and a non-integral regression test were submitted upstream as
...
coverage uses exact multiples of the scale, for which truncation and nearest-even are indistinguishable. A non-integral regression input is required.
...
was approved by ONNX Runtime member

xadupre

on 2026-09-07. The follow-up revision also added an explicit ties-to-even regression case requested during review.

All checks that exercised the change passed. The sole red check,

Optional Lint

, failed before inspecting the changed code: the

reviewdog/action-misspell

container build requested a stale Debian
...
and received HTTP 404 on both attempts. The separate C++ lint, lintrunner, format, build and test jobs passed.
