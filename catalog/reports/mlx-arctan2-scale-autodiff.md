> Archival mirror. [Original report](https://huggingface.co/datasets/XamitK/gero-research-evidence-2026-09/blob/main/mlx-arctan2-scale-autodiff.md). Claims, dates, authorship and licenses remain those of the original publication; this catalog update does not rerun or revalidate its numerical experiments.
...
[Russian audit](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/README.ru.md) · [Native build instructions](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/BUILD.md) · [Patch](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/arctan2-scale-autodiff.patch) · [Fresh publication rerun](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/publication-rerun.json)
...
The [recorded wheel reproduction](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/wheel-reproduction.json) and [full probe](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/probe.py) include the following results at t = 1. Forward values are approximately π/4. The exact first three derivatives are 0.5, −0.5, 0.5.
...
For finite nonzero inputs, the [patch helper](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/scaled_partials.cpp.inc) uses a stopped-gradient scale s = max(|y|,|x|), normalized coordinates u = y/s, v = x/s, and D = u² + v². D lies between 1 and 2. A weighted partial has the exact form g·z/(s²D), with z = x for the y partial and z = −y for the x partial.
...
The [initial normalization artifacts](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/initial-normalization/) passed the first 1350 comparisons. Extending coverage to 1362 revealed six real regressions: cases that passed before the patch and failed after it. For example, float32 y = 2⁻¹²⁰, x = 2³² and cotangent g = 2¹²⁰ require an x partial of approximately −2⁻⁶⁴; premature normalization lost it.

The [two-order weighted artifacts](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/weighted-two-order/) passed 1362 comparisons, but a further extension to 1368 exposed six remaining failures with very small weights. For float32 y = 2⁻⁸⁰, x = 2⁻¹²⁰ and g = 2⁻¹²⁰, the weighted y partial should be approximately 2⁻⁸⁰; the intermediate patch returned zero. These six also failed in the baseline, so they are remaining failures, not new regressions. The final three-order helper passes the extended suite. Earlier scripts and logs are historical evidence; use the top-level portable runner for reproduction.
...
The [native harness](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/native_regression.cpp) tests first-order VJPs and one-sided/joint JVPs across four types, three scales, seven coordinate orientations and multiple weights. It includes 84 radial and 84 angular invariant checks. Float32/64 also cover second and third derivatives, six mixed-Hessian comparisons, noncontiguous matrices and broadcasting from (2,1)/(1,3), with VJP reduction to the original shapes. Weighted anisotropic cases cover bfloat16, float32 and float64.
...
Evidence: [before](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/run-before.json), [after](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/run-after.json), [original build logs](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/build-results.json), [fresh build logs](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/publication-build-results.json), [artifact validator](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/validate_artifacts.py), [artifact validation](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/artifact-validation.json), [source hashes](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/source-metadata.json), [native dependency pins](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/native-source-metadata.json), [checksums](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/SHA256SUMS).
...
[Licenses and attribution](https://www.gero.uz/research/data/2026-09-10-mlx-arctan2-scale-autodiff/LICENSE.md): report and original diagrams CC BY 4.0; code and MLX-derived patches retain MIT notices. Author: Xamit Kadirbekov, GERO Research.


[Published GERO article](https://www.gero.uz/research/articles/mlx-arctan2-scale-autodiff.html) · [GitHub record](https://github.com/kadyrbekovhamit-cyber/gero-numerical-observatory/tree/main/reports/mlx-arctan2-scale-autodiff)
