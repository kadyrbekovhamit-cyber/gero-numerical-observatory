...
void anisotropic(mx::Dtype dtype) {
  int py=dtype==mx::float64 ? -1000 : -120;
  int px=dtype==mx::float64 ? 100 : 32;
  int pw=-py;
  A y(std::ldexp(1.,py),dtype),x(std::ldexp(1.,px),dtype),w(std::ldexp(1.,pw),dtype);
  // The omitted squared-ratio correction is below the reference precision.
  double gy=std::ldexp(1.,pw-px),gx=-std::ldexp(1.,pw+py-2*px);
  check("anisotropic/large-scale/VJP",pack(mx::vjp(fun,{y,x},{w}).second),{gy,gx},dtype);
  SF fx=[=](const A& z){return mx::arctan2(y,z);};
  check("anisotropic/large-scale/JVP-x",mx::jvp(fx,x,w).second,{gx},dtype);
  py=dtype==mx::float64 ? -600 : -80;
  px=dtype==mx::float64 ? -900 : -120;
  pw=dtype==mx::float64 ? 500 : 70;
  A ys(std::ldexp(1.,py),dtype),xs(std::ldexp(1.,px),dtype),ws(std::ldexp(1.,pw),dtype);
  SF fy=[=](const A& z){return mx::arctan2(z,xs);};
  double expected=std::ldexp(1.,pw+px-2*py);
  check("anisotropic/small-scale/JVP-y",mx::jvp(fy,ys,ws).second,{expected},dtype);
  check("anisotropic/small-scale/VJP-y",mx::vjp(fy,ys,ws).second,{expected},dtype);
  px=dtype==mx::float64 ? -1000 : -120;
  pw=px;
  A xtiny(std::ldexp(1.,px),dtype),wtiny(std::ldexp(1.,pw),dtype);
  SF ftiny=[=](const A& z){return mx::arctan2(z,xtiny);};
  expected=std::ldexp(1.,pw+px-2*py);
  check("anisotropic/small-weight/JVP-y",mx::jvp(ftiny,ys,wtiny).second,{expected},dtype);
  check("anisotropic/small-weight/VJP-y",mx::vjp(ftiny,ys,wtiny).second,{expected},dtype);
}
int main() {
  ...
  for(auto dtype:{mx::float16,mx::bfloat16,mx::float32,mx::float64}) {
    ...
    if(dtype!=mx::float16)anisotropic(dtype);
  }
