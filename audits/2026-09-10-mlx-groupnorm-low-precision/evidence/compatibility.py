...
print('phase: numpy import',flush=True)
import numpy as np
print('phase: tests',flush=True)
...
for mode,cls in CLASSES.items():
    print('public test',mode,flush=True)
    ...
    for dtype in [mx.float16,mx.bfloat16,mx.float32,mx.float64]:
        print('membership',mode,str(dtype),flush=True)
...
for dtype in [mx.float16,mx.bfloat16,mx.float32,mx.float64,mx.int32,mx.complex64]:
    for compatible in [False,True]:
        print('unchanged',str(dtype),compatible,flush=True)
...
for dtype in [mx.float16,mx.bfloat16]:
    for label,x in [('empty',mx.zeros((2,0,6),dtype=dtype)),
        ...
        print('edge',str(dtype),label,flush=True)
