...
[Installed-wheel reproduction](evidence/wheel-reproduction.json), [paired before/after reproduction](evidence/paired-reproduction.json), and [reproducer](evidence/reproduce.py) preserve the measurements. Input gradients and gamma gradients are also incorrect in recorded cases; this is not only a display-rounding issue.
...
[Main result rows](evidence/regression-results.json), [compatibility result rows and remaining limits](evidence/compatibility-results.json), and [final compatibility log](evidence/compatibility-final.log) are preserved. Publication preparation checks hashes, row counts, Python syntax and exact patch application; it does not perform a new numerical run. The recorded CPU times are test-suite times, not model-performance benchmarks.
...
The [first patch](evidence/groupnorm-low-precision-v1.patch), [first patched module](evidence/patched-normalization-v1.py), [bounded-run statuses](evidence/bounded-failures.json) and [final empty-input log](evidence/empty-after-final.log) remain in the archive. An earlier test-harness comparison also needed an absolute cancellation tolerance for a tiny float32 derivative residual. That harness correction is documented in [notes](evidence/harness-notes.txt); it is not an MLX defect and did not require changing the numerical patch.
...
See [portable reproduction instructions](BUILD.md), the [original report](evidence/README.md), [static publication verification](publication-verification.json), and [package hashes](SHA256SUMS.json). Original evidence is copied byte for byte. Rerun in a disposable directory because the audit scripts write result files beside themselves. Run commands sequentially on CPU with one numerical thread. No physical-core affinity is claimed.

Report text and original diagrams: CC BY 4.0. MLX source and derivative patches retain MIT terms and copyright notices; see [license details](LICENSE.md) and [MLX license](LICENSE-MLX.txt). Independent, AI-assisted research and publication preparation. The installed MLX and upstream repository were not modified by this publication work.
