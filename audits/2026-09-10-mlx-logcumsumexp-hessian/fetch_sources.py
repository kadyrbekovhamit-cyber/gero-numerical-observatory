"""Read public MLX files and issue metadata only."""
import hashlib
import json
...
def get(url):
    r=subprocess.run(["curl","--fail","--silent","--show-error","--max-time","12",url],
                     capture_output=True,timeout=15)
    if r.returncode:raise RuntimeError(r.stderr.decode().strip())
    return r.stdout
def main():
    ...
    try:
        ...
        for path in ("mlx/primitives.cpp","python/tests/test_autograd.py"):
            try:
    ...
    except Exception as e:meta["errors"].append(dict(path="commits/main",error=str(e)))
    (ROOT/"source-metadata.json").write_text(json.dumps(meta,indent=2)+"\n")
    rows=[]
    for query in ('repo:ml-explore/mlx "logcumsumexp"',
                  'repo:ml-explore/mlx "logcumsumexp" "gradient"',
                  'repo:ml-explore/mlx "logcumsumexp" "hessian"'):
