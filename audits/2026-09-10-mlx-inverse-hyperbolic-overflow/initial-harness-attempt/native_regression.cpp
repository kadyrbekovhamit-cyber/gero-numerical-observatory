...
void check(std::string name,const A& actual,const std::vector<double>& expected,mx::Dtype dtype,double tol=0) {
  ++checks;
  if(!tol)tol=dtype==mx::float64 ? 3e-13 : 8e-6;
...
void first(double input,double weight,bool acosh,mx::Dtype dtype) {
  ...
  A x=mx::astype(A(input),dtype),w=mx::astype(A(weight),dtype);
...
void composite(bool acosh,mx::Dtype dtype,double tvalue) {
  A c=mx::astype(A(std::ldexp(1.,dtype==mx::float64 ? 600 : 80)),dtype);
  A t=mx::astype(A(tvalue),dtype);
  ...
  if(tv==1) {
    ...
    A plus=mx::astype(A(tv+step),dtype),minus=mx::astype(A(tv-step),dtype);
    auto fd=mx::divide(mx::subtract(f(plus),f(minus)),mx::astype(A(2*step),dtype));
