...
def main():
    ...
    (ROOT / "inverse-hyperbolic-real.patch").write_text("".join(difflib.unified_diff(
    ...
    for label, obj, expected in (("before", PREVIOUS/"before.o", 1),
                                  ("after", ROOT/"after.o", 0)):
    ...
    meta = dict(
        ...
        target_methods_identical={
            cls+"::"+name: method(source, cls, name) == method(baseline, cls, name)
            for cls in ("ArcSinh", "ArcCosh") for name in ("jvp", "vjp")},
