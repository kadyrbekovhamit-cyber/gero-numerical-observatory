...
fresh=read("publication-rerun.json")
check("fresh archive matches recorded archive",fresh["archive_sha256"]==native["dependencies"][0]["sha256"])
check("fresh rows match",all(fresh["rows_identical_to_original"].values()))
...
check("fresh compatibility matches", all(v["rows_identical_to_original"] and v["mismatches"]==0 for v in fresh["compatibility"].values()))
